...
default_args = {
    'owner': 'airflow',
    'start_date': dt.datetime(2020, 02, 27, 11, 30, 00),
    'retries': 0
}
...
with DAG('serial_parallel_serial_dag2', default_args=default_args, description='serial - parallel - serial', schedule_interval='@once', catchup=False) as dag:
    	...
    	serial1_task = BashOperator(task_id='s1', bash_command='xcom_push.sh')
    	serial2_task = BashOperator(task_id='s2', bash_command='xcom_pull.sh')
    	execute_parallel0_task 	= BashOperator(task_id='ep', bash_command='sleep 5')
    	parallel1_task 	= BashOperator(task_id='p1', bash_command='sleep 5')
        ...
        parallel3_task 	= BashOperator(task_id='p3', bash_command='sleep 5')
        parallel4_task 	= BashOperator(task_id='p4', bash_command='echo {{ ti.xcom_push("KEY04","VALUE04") }} ')
        execute_serial0_task = BashOperator(task_id='es', bash_command='sleep 5')
